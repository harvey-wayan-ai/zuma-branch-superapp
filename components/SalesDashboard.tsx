...
function SalesSection({ title, items, filter }: SalesSectionProps) {
  ...
  return (
    <div className="zuma-card mb-4 overflow-hidden">
      <div 
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left text-xs text-gray-500 border-b border-gray-100">
                ...
                <th className="py-3 px-4 font-medium text-right">vs. Last</th>
              </tr>
            </thead>
            <tbody className="text-sm">
              {items.map((item, index) => (
                <tr key={index} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-gray-700 font-medium">{item.value}</td>
                  <td className="py-3 px-4 text-right">
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getChangeColor(item.change)}`}>
              ...
              <tr className="bg-gray-50 font-semibold">
                ...
                <td className="py-3 px-4 text-gray-900">RpXX B</td>
                <td className="py-3 px-4 text-right">
                  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium text-green-500 bg-green-50">
                    10.5%
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 mb-4">
        <button className="flex items-center gap-1 px-3 py-2 bg-white rounded-lg text-sm font-medium text-gray-700 shadow-sm border border-gray-200">
          {metricFilter}
          <ChevronDown className="w-4 h-4" />
        </button>
        <button className="flex items-center gap-1 px-3 py-2 bg-white rounded-lg text-sm font-medium text-gray-700 shadow-sm border border-gray-200">
      ...
      {/* Revenue Card */}
      <div className="zuma-card p-4 bg-gradient-to-br from-[#0D3B2E] to-[#1a5a48] text-white">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="flex gap-1">
            ...
            <span className="text-xs bg-white/20 px-2 py-1 rounded">This Week</span>
